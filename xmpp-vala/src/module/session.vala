...
public class Module : XmppStreamNegotiationModule {
    public static ModuleIdentity<Module> IDENTITY = new ModuleIdentity<Module>(NS_URI, "session_module");
    ...
    private void on_bound_resource(XmppStream stream, string my_jid) {
        if (stream.features.get_subnode("session", NS_URI) != null) {
            ...
            Iq.Stanza iq = new Iq.Stanza.set(new StanzaNode.build("session", NS_URI).add_self_xmlns()) { to=get_bare_jid(my_jid) };
